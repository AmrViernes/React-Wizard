...
export const Wizard = <T, S extends string>({
  ...
  const parentTestId = props['data-testid'] || 'wizard';
  const [data, setData] = useState<Partial<T>>(initialData);
  const steps = Object.keys(stepMapping) as S[];
  const [step, setStep] = useState<S>(steps[0]);

  useEffect(() => {
    if (trackSteps && typeof window !== 'undefined') {
      const event = new CustomEvent('wizard:step-change', {
        detail: { step, stepIndex: steps.indexOf(step) },
      });
      window.dispatchEvent(event);
    }
  }, [step, steps, trackSteps]);
  ...
  if (!CurrentStepComponent) {
    return (
      <div data-testid={`${parentTestId}-not-found`}>Step not found</div>
    );
  }
  ...
  return (
    ...
    >
      <CurrentStepComponent
        ...
        data-testid={`${parentTestId}-step-${step}`}
      />
    </WizardProvider>
  );
};